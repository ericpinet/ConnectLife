...
public class CmdDeleteAssetTest {
	...
	public void testNullAsset() {
		CmdDeleteAsset cmd = CmdFactory.getCmdDeleteAsset(null);
		try {
			cmd.execute();
			fail();
		} catch (Exception e) {
	...
	public void testWithoutUidAsset() {
		CmdDeleteAsset cmd = CmdFactory.getCmdDeleteAsset(new Asset(null, "test", AssetType.IMAGE, AssetMode.USER));
		try {
			cmd.execute();
			fail();
		} catch (Exception e) {
		...
		CmdDeleteAsset cmd2 = CmdFactory.getCmdDeleteAsset(new Asset("", "test", AssetType.IMAGE, AssetMode.USER));
		try {
			cmd2.execute();
			fail();
		} catch (Exception e) {
	...
	public void testNotFound() {
		...
		CmdDeleteAsset cmd = CmdFactory.getCmdDeleteAsset(new Asset("12345","", AssetType.IMAGE, AssetMode.USER));
	...
	public void testComplete() {
		...
		CmdDeleteAsset cmd = CmdFactory.getCmdDeleteAsset(new Asset("12345","test", AssetType.IMAGE, AssetMode.USER));
		cmd.setContext(context);
		try {
			cmd.execute();
